...
static void step(ap_uint<1> rx_dv,
                 ap_uint<4> rxd,
                 ap_uint<1> rxerr,
                 ap_uint<1> &tx_en,
                 ap_uint<4> &txd,
...
static std::vector<uint8_t> collect_tx_frame(unsigned max_cycles) {
    ...
    ap_uint<1> tx_en = 0, rx_accept = 0, tx_frame = 0, rx_active = 0, tx_active = 0;
    ap_uint<4> txd = 0;
    bool in_frame = false;
    bool low = false;
    uint8_t byte = 0;
    ...
    for (unsigned i = 0; i < max_cycles; ++i) {
        ...
        if (tx_en) {
            ...
            for (unsigned drain = 0; drain < 64; ++drain) {
                step(0, 0, 0, tx_en, txd, rx_accept, tx_frame, rx_active,
                     tx_active);
            }
            break;
        }
...
static void assert_valid_test_frame(const std::vector<uint8_t> &frame,
    ...
    if (frame.size() != 8 + body_len + 4) {
        std::fprintf(stderr, "bad test frame size for payload %u: expected %u got %u\n",
                     payload_len, 8 + body_len + 4, (unsigned)frame.size());
    }
    assert(frame.size() == 8 + body_len + 4);
    assert_valid_tx_frame(frame,
...
static void send_rx_frame(const std::vector<uint8_t> &frame) {
    ...
    for (uint8_t byte : frame) {
        step(1, byte & 0x0f, 0, tx_en, txd, rx_accept, tx_frame, rx_active, tx_active);
        step(1, byte >> 4, 0, tx_en, txd, rx_accept, tx_frame, rx_active, tx_active);
    }
    step(0, 0, 0, tx_en, txd, rx_accept, tx_frame, rx_active, tx_active);
}
...
int main() {
    ...
    assert_valid_tx_frame(broadcast_ack,
                          {0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f},
                          {'A', 'R', 'T', 'Y', '_', 'A', 'C', 'K'});
